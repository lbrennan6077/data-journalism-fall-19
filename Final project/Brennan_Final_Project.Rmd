...
#Answer: Not that strong. This value tells us that _________.
...
test <- cor.test(balcity_population_and_DEA$pills_per_person, balcity_population_and_DEA$estimate, method="pearson")

#Question 19: How strong is the relationship?

#Again, not very strong. We'll have to see how this plays out nationally to determine whether we're just running around in circles and there's no actual trend — on the county level, or nationally — whatsoever, or whether there is a trend nationally, and Baltimore County and city are departing from it. 
...
#POSSIBLE ERRORS: might leave ZIP code error, tract data for pharmacies not perscribers, compare to other counties
...
#FINDINGS ABOUT HOWARD RELATIONSHIP
  
test <- cor.test(HoCo_population_and_DEA$pills_per_person, HoCo_population_and_DEA$estimate, method="pearson")

print(test)

#Question 23: FINDINGS ABOUT STRENGTH OF HOWARD RELATIONSHIP
...
test <- cor.test(PG_population_and_DEA$pills_per_person, PG_population_and_DEA$estimate, method="pearson")
...
test <- cor.test(AA_population_and_DEA$pills_per_person, AA_population_and_DEA$estimate, method="pearson")
...
test <- cor.test(CC_population_and_DEA$pills_per_person, CC_population_and_DEA$estimate, method="pearson")
