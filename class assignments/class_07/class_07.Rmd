...
```{r}
  ...
  labs(x="Year", y="Total pills", title="In Logan County, opioid shipments stay steadily high from \n2006 to 2012", subtitle = "Total pills shipped to Logan County by year", caption = "Source: DEA ARCOS database, via Washington Post") +
  scale_x_continuous(breaks = c(2006, 2007, 2008, 2009, 2010, 2011, 2012)) +
  scale_y_continuous(labels = comma)

#Despite slight dips in 2006 and 2010, opioid shipments stayed steadily high from 2006 to 2012. We might want to look into why they slightly dipped in 2006 and 2010.
```
...
```{r}
  ...
  labs(x="Year", y="Total pills", title="Kanawha County claims highest rise in opioid shipments \n among six West Virginia counties", subtitle = "Total pills per year shipped to Kanawha, Cabell, Mingo, \n Raleigh, Logan and Wood counties", caption = "Source: DEA ARCOS database, via Washington Post", fill="County") +
  scale_x_continuous(breaks = c(2006, 2007, 2008, 2009, 2010, 2011, 2012)) +
  scale_y_continuous(labels = comma) +
...
#Six West Virginia counties show high totals for opioid shipments, but Kanawha County showed the largest increase among them. Mingo saw the most dramatic decrease, beginning in 2010 and continuing through 2012. 
...
```{r}
#the relationship between the following two factors for Maryland counties: the pills per person rate (not total pills, but pills per person) for a county in 2012 and the county's median household income in 2012.  

maryland_county_pills_2012 <- arcos_county_pills_per_year %>%
...
maryland_pill_pop_join <- maryland_county_pills_2012 %>%
  inner_join(maryland_population_2012, by="countyfips")

#Next, per person

maryland_pills_by_person <- maryland_pill_pop_join %>%
  mutate(maryland_pills_by_person = dosage_unit/population) 

#join by GEOID
maryland_2012_countyfips <- maryland_pills_by_person %>%
...
```{r}
# Scatterplot that explores the relationship between the following two factors for Maryland counties: the pills per person rate (not total pills, but pills per person) for a county in 2012 and the county's median household income in 2012.
...
ggplot(maryland_2012_countyfips) +
  geom_point(aes(estimate, maryland_pills_by_person)) +
  labs(x="County's median household income", y="Pills per person rate in 2012", title="Pills per person and median household income\n  held steady in 2012 (despite my cursed line)", caption = "Source: DEA ARCOS database, via Washington Post", fill="buyer_county") +
  scale_y_continuous(labels = comma) +
  scale_x_continuous(labels = comma) + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  geom_smooth(aes(population, dosage_unit), method = "lm", se = FALSE) +
  geom_text_repel(aes(estimate,maryland_pills_by_person, label=buyer_county.x),
                  subset(maryland_2012_countyfips, maryland_pills_by_person > 35))

#I worked on this for a while to adjust the axis scales, to no avail. Any tips?
...
```{r}
  ...
  geom_bar(stat="identity", aes(year, dosage_unit, fill=buyer_county)) +
  labs(x="Year", y="Total pills", title="Baltimore County had a steeper proportion of pills\n shipped over six years", subtitle = "Total pills per year shipped to Baltimore City and Baltimore County", caption = "Source: DEA ARCOS database, via Washington Post", fill="County") +
  scale_x_continuous(breaks = c(2006, 2007, 2008, 2009, 2010, 2011, 2012)) +
  scale_y_continuous(labels = comma) 
